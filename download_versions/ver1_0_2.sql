...
CREATE OR REPLACE FUNCTION sotr_game.drop_game()
 RETURNS text
 LANGUAGE plpgsql
AS $function$
begin
	
	truncate sotr_game.g_enemy restart identity cascade;
	truncate sotr_game.g_inventory restart identity cascade;
	truncate sotr_game.game_statistic restart identity cascade;
	truncate sotr_game.saves restart identity cascade;
